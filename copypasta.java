...
public class CalculateDamLevel {
	 ...
	 *
	 * @param date is day.month.year (all in string decimal formats), then 2
	 *             doubles.
	 */
	public static double calculateNextState(String day, String month, String year, double flowInput,
			double levelInput) {
		...
		// myData.setPrecipitationLevel(2.9); //FIX THIS! --> Should be fixed...? I
		// think.
	...
	public static double precipValue(GregorianCalendar date) {
		...
		temp.readCSV("eng-daily-01012019-12312019_dailyPrecip_calgary.csv");
	...
	public static void main(String[] args) {
		// tDay.set(2019, 0, 17);
		calculateNextState("17", "1", "2019", 50, 1078);
	}
	...
	void calculate() {
		...
		System.out.println("reservoirTen" + outsideAverageReservoir(previousTen, averageDay));
		System.out.println("reservoirDay" + outsideAverageReservoir(myDailyData, averageDay));
		System.out.println("precipTen" + outsideAveragePrecipitationLevel(previousTen, averageDay));
		System.out.println("precipDay" + outsideAveragePrecipitationLevel(myDailyData, averageDay));
		System.out.println("FlowTen" + outsideAverageFlowRate(previousTen, averageDay));
		System.out.println("flowDay" + outsideAverageFlowRate(myDailyData, averageDay));
	...
	double outsideAverageReservoir(DailyData actual, DailyData expected) {
		/*
		 * System.out.println("upper: " + (expected.getReservoirLevel() - 5 + (5 *
		 * (myAcceptableReservoirPercent + 1)))); System.out.println("lower: " +
		 * (expected.getReservoirLevel() - 5 + (5 * (1 -
		 * myAcceptableReservoirPercent)))); if (actual.getReservoirLevel() >
		 * (expected.getReservoirLevel() + (expected.getReservoirLevel() *
		 * myAcceptableReservoirPercent))) { double myVal = actual.getReservoirLevel() -
		 * (expected.getReservoirLevel() + (expected.getReservoirLevel() *
		 * myAcceptableReservoirPercent)); changeValue += myVal; return myVal; } else if
		 * (actual.getReservoirLevel() < (expected.getReservoirLevel() -
		 * (expected.getReservoirLevel() * myAcceptableReservoirPercent))) { double
		 * myVal = actual.getReservoirLevel() - (expected.getReservoirLevel() -
		 * (expected.getReservoirLevel() * myAcceptableReservoirPercent)); changeValue
		 * += myVal; return myVal; } return 0;
		 */
...
//	private static GregorianCalendar tDay= new GregorianCalendar();
	...
	DailyData getPreviousNDayData(GregorianCalendar theDay, int n) {
		...
		for (int i = 0; i < n; i++) {
			...
			// previousArr[i].setFlowRate(
			// Double.parseDouble(myFlowRate.getSpecifiedDate(theDay)));
			previousArr[i].setReservoirLevel(myDamWaterLevel.getAverageSpecifiedDate(theDay));
			// previousArr[i].setReservoirLevel(
			// Double.parseDouble(myDamWaterLevel.getAverageSpecifiedDate(theDay)));
